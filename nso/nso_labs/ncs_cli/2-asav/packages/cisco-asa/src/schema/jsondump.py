...
class XPathEval(object):
    ...
    def expand_paths(self, stmt):
        in_scope = True
        if self.oper:
            in_scope &= self.expr1.expand_paths(stmt)
            if (self.expr2 and
                ((type(self.expr2.expr1) != type("")) or
                 (self.oper in ["and", "or"]))):
                in_scope &= self.expr2.expand_paths(stmt)
        else:
            if self.expr1.startswith("/"):
                in_scope = False
                self.expr1 = re.sub("/[a-z]+:(.*)", "/\\1", self.expr1)
            else:
                rel_path = self.expr1
                is_hide = False
                ...
                while(rel_path and rel_path.startswith("../")):
                    ...
                    if stmt.is_context():
                        break
                in_scope = not (rel_path.startswith("..") or
                                (stmt.is_context() and not is_hide))
                rel_path = stmt.path() + "/"  + rel_path
                if ("#" in rel_path):
...
_data_keywords = {
    ...
    ("tailf-common", "meta-data")
}

def stmt_to_dict(self, flatten_level=-1):
    if self.arg == None:
        name = None
    ...
    for s in self.substmts:
        if not s.keyword in _data_keywords:
...
def extract_type(stmt, jsondumper):
    ...
    if typename == "empty":
        ...
        return extract_type(ref, jsondumper)
...
class Node(object):
    ...
    def emit(self):
        dump = self.stmt.to_dict(flatten_level=1)
        if self.type:
    ...
    def emit_cli(self, dump):
        ...
        if defval:
            ...
            dump["is_optional"] = "true"
        if hasattr(self, "key_separator"):
            dump["key_separator"] = self.key_separator
            ...
            if in_uses and parentuses:
                ...
                if len(when) > 0:
                    if len(when) == 1:
                        ...
                        if xpatheval.expand_paths(self.stmt.real_parent()) or force_include_when:
                            dump["when"] = xpatheval.show()
                            if force_include_when:
                                dump["force_when"] = "true"
                        else:
...
#             self.jsondumper.add_warning("EMPTYDROP_NODE", stmt)
    ...
    def extract_list(self, stmt):
        if self.jsondumper.ctx.opts.json_cli:
            ...
            if keyfmt:
                m = re.match("\\$\\(1\\)(.)\\$\\(2\\)", keyfmt.arg)
                if m:
                    self.key_separator = m.group(1)
                else:
